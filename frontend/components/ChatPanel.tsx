...
export function ChatPanel({
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    try {
      const result = await sendChatMessage(nextMessages, documentType, documentId);
    ...
    <div className="no-print flex flex-col gap-4 p-6 lg:h-screen lg:min-h-0">
      ...
      <div className="flex-1 overflow-y-auto rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <ul className="flex flex-col gap-3">
          ...
          {isSending && (
            <li className="max-w-[85%] rounded-lg bg-zinc-100 px-3 py-2 text-sm text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
              …
            </li>
          )}
        </ul>
